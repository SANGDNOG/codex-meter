...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    const result = await inspectRollouts({ root: explicitSessions || undefined, home: explicitSessions ? undefined : codexHome(option(args, '--codex-home')), rawIds: has(args, '--raw-session-ids'), secretFile: option(args, '--secret-file') || undefined });
