...
export class MeterStore {
  ...
  start(token) { return this.mutate((s) => {
    ...
    if (user.used.total_tokens >= s.config.quotaTokens) return { status: 403, body: { error: 'quota_exhausted', used: user.used.total_tokens, quota: s.config.quotaTokens } };
    ...
    return { status: 201, body: { leaseId: id, quota: s.config.quotaTokens, used: user.used.total_tokens, leaseTtlMs: s.config.leaseTtlMs } };
  }); }
  applyUpdate(s, token, leaseId, absolute, finishing = false) {
    ...
    const crossed = user.used.total_tokens >= s.config.quotaTokens;
    ...
    return { status: 200, body: { stop, reason: !user.enabled ? 'user_disabled' : crossed ? 'quota_exhausted' : expired ? 'lease_expired' : null, used: user.used, quota: s.config.quotaTokens } };
  }
  ...
  usage(token) { return this.mutate((s) => { this.maintain(s); const user = this.authenticate(s, token); return user ? { status: 200, body: { user: user.id, used: user.used, quota: s.config.quotaTokens, periodStart: s.periodStart, resetPeriodMs: s.config.resetPeriodMs, enabled: user.enabled } } : { status: 401, body: { error: 'bad_auth' } }; }); }
