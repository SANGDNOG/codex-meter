...
test('M7 release manifest and checksums are deterministic and workflow enforces native SEA smoke tests', async () => {
  ...
  try {
    ...
    const manifest=JSON.parse(first); assert.equal(manifest.version,'2.1.3'); assert.deepEqual(Object.keys(manifest.artifacts),['linux-x64','macos-arm64','windows-x64']); assert.equal(sums.trim().split('\n').length,3);
