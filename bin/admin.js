...
if (command === 'init') {
  ...
  const quotaTokens = Number(opts['--quota']); const resetPeriodMs = Number(opts['--reset-ms']); const maxConcurrentLeases = Number(opts['--max-leases'] || 1); const leaseTtlMs = Number(opts['--lease-ttl-ms'] || 120000);
  if (![quotaTokens, resetPeriodMs, maxConcurrentLeases, leaseTtlMs].every(Number.isSafeInteger) || [quotaTokens, resetPeriodMs, maxConcurrentLeases, leaseTtlMs].some((n) => n <= 0)) fail('positive integers required: --quota, --reset-ms, --max-leases, --lease-ttl-ms');
  ...
  await atomicWriteJson(stateFile, { version: 1, periodStart: Date.now(), config: { quotaTokens, resetPeriodMs, maxConcurrentLeases, leaseTtlMs }, adminTokenHash: hashToken(adminToken), users: records, leases: {} });
...
} else fail('usage: admin.js init --users=a,b,c --quota=N --reset-ms=N [--max-leases=N] [--lease-ttl-ms=N]\n       admin.js set-enabled USER true|false');
